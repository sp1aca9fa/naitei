    ...
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold text-amber-800">{t('dashboard.resumeCard.notSetUp')}</p>
          <p className="text-xs text-amber-700 mt-0.5">{t('dashboard.resumeCard.notSetUpDesc')}</p>
          <Link to="/profile/resume" className="text-xs font-medium text-amber-800 underline mt-2 inline-block hover:text-amber-900">{t('dashboard.resumeCard.setUpLink')}</Link>
        </div>
      </div>
    )
  }
        ...
        <div className="min-w-0 space-y-1">
          {lastUpdated && (
            <div className="flex gap-2 items-end">
              <p className="text-sm font-semibold text-gray-600 uppercase tracking-wide leading-none">{t('dashboard.resumeCard.label')}</p>
              <span className="text-xs text-gray-400 italic leading-none">{t('dashboard.resumeCard.lastUpdated', { date: lastUpdated })}</span>
            </div>
          )}
      ...
      {(activeVersion?.key_strengths?.length || activeVersion?.focus_skills?.length) ? (
        <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
          {activeVersion?.key_strengths && activeVersion.key_strengths.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">{t('dashboard.resumeCard.keyStrengths')}</p>
                <span className="relative group">
                  ...
                  <span className="pointer-events-none absolute left-0 top-full mt-1.5 z-20 w-64 rounded bg-gray-800 px-2.5 py-1.5 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity leading-relaxed">
                    {t('dashboard.resumeCard.keyStrengthsTooltip')}
                  </span>
                </span>
              </div>
          ...
          {activeVersion?.focus_skills && activeVersion.focus_skills.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">{t('dashboard.resumeCard.focusSkills')}</p>
                <span className="relative group">
                  <span className="w-3.5 h-3.5 rounded-full bg-gray-200 text-gray-500 text-[10px] flex items-center justify-center cursor-help font-medium select-none">?</span>
                  <span className="pointer-events-none absolute left-0 top-full mt-1.5 z-20 w-72 rounded bg-gray-800 px-2.5 py-1.5 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity leading-relaxed">
                    {t('dashboard.resumeCard.focusSkillsTooltip')}
                  </span>
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
...
export function DashboardPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-12 space-y-6">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-orange-800">
                  {t('dashboard.alerts.followUp', { count: stats.followUpNeeded.length })}
                </p>
                <div className="mt-1 space-y-0.5">
                  {stats.followUpNeeded.slice(0, 2).map(item => (
                    <p key={item.id} className="text-xs text-orange-700">
                      ...
                      {item.company && <span> {t('common.atCompany', { company: item.company })}</span>}
                      <span className="text-orange-500"> — {t('dashboard.alerts.daysAgo', { count: item.daysSince })}</span>
                    </p>
                  ))}
                  {stats.followUpNeeded.length > 2 && (
                    <Link to="/applications" className="text-xs text-orange-600 underline">
                      {t('dashboard.alerts.more', { count: stats.followUpNeeded.length - 2 })}
                    </Link>
                  )}
                </div>
              </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-blue-800">
                  {t('dashboard.alerts.interviewPrep', { count: stats.interviewPrepNeeded.length })}
                </p>
                <div className="mt-1 space-y-0.5">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">{t('dashboard.topMatches.title')}</p>
        {!stats ? (
          <p className="text-sm text-gray-300">{t('common.loading')}</p>
        ) : stats.topUnappliedMatches.length === 0 ? (
          <p className="text-sm text-gray-400">
            {t('dashboard.topMatches.empty')}{' '}
            <Link to="/jobs" className="underline hover:text-gray-600">{t('dashboard.topMatches.emptyLink')}</Link>.
          </p>
        ) : (
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-xs text-gray-400 uppercase tracking-wide mb-4">{t('dashboard.thisWeek.title')}</p>
          {!stats ? (
            <p className="text-sm text-gray-300">{t('common.loading')}</p>
          ) : (
            <div className="space-y-3">
              {([
                { key: 'analyzed', value: stats.weeklyStats.analyzed },
                { key: 'applied', value: stats.weeklyStats.applied },
                { key: 'interviews', value: stats.weeklyStats.interviews },
              ] as const).map(({ key, value }) => (
                <div key={key} className="flex items-baseline justify-between">
                  <span className="text-xs text-gray-500">{t(`dashboard.thisWeek.${key}`)}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-xs text-gray-400 uppercase tracking-wide mb-4">{t('nav.pipeline')}</p>
          {!stats ? (
            <p className="text-sm text-gray-300">{t('common.loading')}</p>
          ) : (
                ...
                return stageList.map(stage => {
                  ...
                  return (
                    <div key={stage} className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 w-16 flex-shrink-0">{t(`dashboard.stages.${stage}`)}</span>
      ...
      {skillGaps && skillGaps.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('dashboard.skillGaps.title')}</p>
            <Link to="/insights" className="text-xs text-blue-500 hover:text-blue-700">{t('dashboard.skillGaps.fullAnalysis')}</Link>
          </div>
              ...
              <div key={gap.skill} className="space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('dashboard.skillGaps.requiredBy')}</span>
                    <span className="font-medium text-gray-700">{t('dashboard.skillGaps.jobs', { count: gap.frequency })}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('dashboard.skillGaps.avgMatch')}</span>
                    <span className={`font-medium ${scoreColor(gap.avg_score)}`}>{gap.avg_score}%</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('dashboard.skillGaps.impact')}</span>
